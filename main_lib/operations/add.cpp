...
namespace bigNum{
    bignum bignum::add(const bignum& a, const bignum& b){
        ...
        int a_from_dot = static_cast<int>(a.number.length()) - a.real_size - 1;
        int b_after_zero = static_cast<int>(b.number.length()) - b.real_size - 1;
        int after_zero = std::max(a_from_dot, b_after_zero);
        int mind = 0;
        for(int i = after_zero; i > 0; i--){
            if(i <= a_from_dot){
